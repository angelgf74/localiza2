...
- [x] **Corregir `CLAUDE.md`: almacenamiento de token** — dice "Android DataStore / web localStorage"; código real usa `EncryptedSharedPreferences` (Android, verificado en `SessionManager.kt`) y `sessionStorage` (web, verificado en `app.js`/`admin.js`/`pair.js`). Aprovechado para documentar también `TokenVersion`/claim `tv`.
- [x] **Depurar `mejoras.md`** — 9 de 12 ítems verificados como ya implementados en código actual y movidos a `## Hecho` (con fecha y referencia al fichero). Quedan pendientes de verdad: #5 (refresh token — nota: expira a 30 días, no 24h como decía el ítem original; la revocación ya está resuelta vía `TokenVersion`) y #10 (geofencing, sin implementar).
